    ...
    if(data.success && data.units.length) {
      unitsList.innerHTML = data.units.map(unit => `
        <li class="card" style="margin-bottom:1rem;">
          <h3><i class="fas fa-book-open"></i> ${unit.unit_name || unit.description || 'Unit'}</h3>
...
toggleLink.addEventListener('click', (e) => {
  ...
  if (formAction.value === 'login') {
    ...
    document.querySelector('#auth-form button').textContent = 'Registrieren';
  } else {
    ...
    document.querySelector('#auth-form button').textContent = 'Einloggen';
  }
});
